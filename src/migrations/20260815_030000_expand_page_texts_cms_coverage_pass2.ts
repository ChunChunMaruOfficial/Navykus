...
export async function up({ db }: MigrateUpArgs): Promise<void> {
  ...
  for (const pageOption of EDITABLE_PAGE_TEXT_PAGES) {
    ...
    for (const [index, translationKey] of keys.entries()) {
      const sourceValue = defaultLocale[translationKey];
      if (typeof sourceValue !== 'string') continue;

      await insertPageText({ db, page, translationKey, value: sourceValue, sortOrder: index });
      ...
      if (String(source.value || '').trim() !== sourceValue.trim()) continue;
