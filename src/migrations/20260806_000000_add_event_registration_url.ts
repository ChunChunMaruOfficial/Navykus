...
export async function up({ db }: MigrateUpArgs): Promise<void> {
  await addColumnIfMissing(db, 'events', 'registration_url', 'text');
  await addColumnIfMissing(db, '_events_v', 'version_registration_url', 'text');
}

export async function down({ db }: MigrateDownArgs): Promise<void> {
  await dropColumnIfExists(db, '_events_v', 'version_registration_url');
  await dropColumnIfExists(db, 'events', 'registration_url');
}
