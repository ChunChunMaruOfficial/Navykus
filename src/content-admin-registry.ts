...
export const ADMIN_CONTENT_TYPES = [
  ...
  },
  {
    ...
    fields: [
      ...
      { name: 'suitableFor', label: 'Кому подходит', type: 'textarea' },
      { name: 'format', label: 'Формат участия', type: 'textarea', help: 'Первая строка - крупный текст в карточке, остальные строки - подпись. Пустое поле скрывает карточку.' },
      ...
      { name: 'themesText', label: 'Темы кейса', type: 'textarea' },
      { name: 'evaluationCriteriaText', label: 'Критерии оценки', type: 'textarea' },
      ...seoFields,
    ],
  },
  {
    ...
    fields: [
      ...
      { name: 'eventDate', label: 'Дата события', type: 'date', required: true },
      { name: 'displayDate', label: 'Дата на сайте', type: 'text', help: 'Необязательно. Если заполнено, сайт покажет этот текст вместо автоматической даты.' },
      { name: 'showTime', label: 'Показывать время на сайте', type: 'checkbox', defaultValue: false },
      { name: 'timeZone', label: 'Time zone', type: 'text', defaultValue: 'Europe/Moscow' },
      { name: 'registrationDeadline', label: 'Дедлайн регистрации', type: 'date' },
      ...
      { name: 'outcomesText', label: 'Что вы получите', type: 'textarea', help: 'Один пункт на строку.' },
      { name: 'prerequisites', label: 'Предварительный опыт', type: 'textarea' },
      ...seoFields,
    ],
  },
  {
  ...
  },
  {
    ...
    fields: [
      ...
      { name: 'benefits', label: 'Что вы получите', type: 'list' },
  ...
  },
  {
    ...
    fields: [
      ...legacyPublishedFields,
      { name: 'page', label: 'Страница', type: 'select', required: true, options: ['global', 'home', 'about', 'championship', 'activities', 'find-team', 'legal'], defaultValue: 'home' },
      { name: 'translationKey', label: 'Служебный ключ', type: 'text', required: true },
      { name: 'label', label: 'Где используется', type: 'text', required: true },
      { name: 'value', label: 'Русский текст', type: 'textarea' },
    ],
  },
  {
    key: 'pillars',
