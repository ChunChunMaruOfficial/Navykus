...
// Popular world countries (CIS + Middle East + rest of the world) shown in the
// registration form dropdown, so users can pick their country even before any
// team members exist in the CMS. Merged with countries from existing members.
    ...
    <AnimatePresence>
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
        ...
        <motion.div
          ...
          transition={{ type: 'spring', damping: 25, stiffness: 220 }}
          className="relative z-10 max-h-[calc(100vh-2rem)] w-[96%] max-w-4xl overflow-y-auto rounded-3xl border border-white/60 bg-white/35 p-6 shadow-[inset_0_1.5px_3px_rgba(255,255,255,0.45),0_40px_120px_rgba(27,24,22,0.12)] backdrop-blur-3xl sm:w-full sm:p-8"
        >
          <button
            onClick={onClose}
            className="absolute right-4 top-4 z-20 rounded-full p-2 text-brand-dark transition-colors hover:bg-brand-bg-3/50"
            aria-label={t('ui.applicationmodal.877618185f')}
          >
            <X className="h-5 w-5" />
          </button>
          <TeamMemberApplicationForm context={{ sourceType: 'find-team', sourceTitle: t('ui.app.d13f387e64') }} />
        </motion.div>
      </div>
    </AnimatePresence>
  );
